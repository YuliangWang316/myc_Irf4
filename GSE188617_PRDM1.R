...
for (h in c(5,5.5,6)) {
  for (k in c(3000,2500)) {
    
  

pbmcs <- subset(pbmc, subset = nFeature_RNA > 200 & nFeature_RNA < k & percent.mt < h)
...
remove(KDM6B,LY75,MYC,P,PRDM1,all.genes,pbmcs,h,i,idents,j,k,noise,S)
...
pbmcK <- FindNeighbors(pbmcn, dims = 1:4, verbose=F)
pbmcK <- FindClusters(pbmcK, resolution = 0.1, verbose=F)
DotPlot(pbmcK, features = c("MYC","PRDM1","KDM6B","LY75","Mtor_1","Mtor_2","Mtor_3","IRF4")) + RotatedAxis()
VlnPlot(pbmcK,features = c("MYC","PRDM1","KDM6B","LY75","Mtor_1","Mtor_2","Mtor_3","IRF4"),pt.size = 0,sort = TRUE)
