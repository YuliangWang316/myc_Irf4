...
pbmcn <- subset(pbmc, subset = nFeature_RNA > 200 & nFeature_RNA < 3000 & percent.mt < 5)
...
remove(Glutamine_1,Glutamine_1_new,Glutamine1,all.genes,average_Glutamine1)
pbmcn <- FindNeighbors(pbmcn, dims = 1:3, verbose=F)
pbmcn <- FindClusters(pbmcn, resolution = 0.1, verbose=F)
DotPlot(pbmcn, features = c("MYC","IRF4","KDM6B","LY75","Mtor_1","Mtor_2","Mtor_3","Glutamine_1")) + RotatedAxis()
VlnPlot(pbmcn,features = c("MYC","IRF4","KDM6B","LY75","Mtor_1","Mtor_2","Mtor_3","Glutamine_1"),sort = TRUE,pt.size = 0)
