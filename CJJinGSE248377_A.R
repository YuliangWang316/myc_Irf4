...
ref<-merge(ref1, y = c(ref3), add.cell.ids = c("ref1", "ref3"), project = "Total")
...
gc()
saveRDS(pbmc,file = "e:/CJJGSE248377_A.rds")
...
ref_new<-RunUMAP(ref_new,reduction = "lda",reduction.name = "ldaumap",dims = 1:6)
ref_new<-RunTSNE(ref_new,reduction = "lda",reduction.name = "ldatsne",dims = 1:6)
...
remove(all.genes)
new.cluster.ids <- c("LZ","DZ","prePB","PC","LZ","DZ","LZ")
...
saveRDS(ref_new,"e:/CJJGSE248377_A_new.rds")
...
write.table(diff_test_res,file = "diff_test_res_GSE248377_A.txt",sep = "\t",row.names = FALSE,quote = FALSE)
remove(diff_test_res)

saveRDS(monocle_cds,"e:/CJJGSE243877_A_monocle.rds")
