...
ref<-subset(ref,idents = c("LZ GC","GC","DZ GC","FCRL2/3high GC","prePB","Plasmablast","Cycling"))
...
pbmc<-RunUMAP(pbmc,reduction = "lda",reduction.name = "ldaumap",dims = 1:6)
pbmc<-RunTSNE(pbmc,reduction = "lda",reduction.name = "ldatsne",dims = 1:6)
...
remove(pbmc2)
gc()
saveRDS(pbmc,"e:/CJJEMTAB9005.rds")
remove(pbmc)
gc()

pbmc<-ref
pbmc.metadata<-ref.metadata
remove(ref,ref.metadata)
gc()
...
pbmc<-RunUMAP(pbmc,reduction = "lda2",reduction.name = "lda2umap",dims = 1:9)
pbmc<-RunTSNE(pbmc,reduction = "lda2",reduction.name = "lda2tsne",dims = 1:9)
Idents(pbmc)<-pbmc$newcluster
DimPlot(pbmc,reduction = "lda2umap")
...
saveRDS(pbmc,"e:/EMTAB9005smallformonocle.rds")
...
saveRDS(pbmc,"e:/EMTAB9005bigformonocle.rds")
...
saveRDS(monocle_cds,"d:/CJJ_HUMANpublic_pseudotime/EMTAB9005/Monocle2_Sampling.rds")
