...
cth <- addCellType(cth, "Myc_Irf4", classify_func =
                     function(x) { x[Myc_id,] >= mean(Myc$Myc) & x[Irf4_id,] >= mean(Irf4$Irf4) })
cth <- addCellType(cth, "Myc", classify_func =
                     function(x) { x[Myc_id,] >= mean(Myc$Myc) & x[Irf4_id,] < mean(Irf4$Irf4) })
cth <- addCellType(cth, "non_Myc_Irf4", classify_func =
...
plot_cell_clusters(monocle_cds,1,2,color_by = "Cluster" )
Idents(pbmc1)<-monocle_cds@phenoData@data[["Cluster"]]
VlnPlot(pbmc1,features = c("Myc","Irf4","Kdm6b","Ly75"),sort = TRUE,pt.size = 0)
DotPlot(pbmc1, features =c("Myc","Irf4","Kdm6b","Ly75")) + RotatedAxis()
Myc1<-subset(pbmc1,idents="6")
remove(pbmc)
remove(cth,data,fd,fData,Irf4,marker_diff,marker_spec,monocle_cds,Myc,pd,candidate_clustering_genes,counts,expressed_genes,Features,Irf4_id,Myc_id,percentmt,semisup_clustering_genes)
pbmc2<-subset(pbmc1,idents=c("1","2","3","4","5","7","8","9"))
data<-as.matrix(pbmc2@assays$RNA@counts)
pd <-pbmc2@meta.data
pd <- new('AnnotatedDataFrame', data = pd)  
fData<-data.frame(gene_short_name = row.names(data),row.names = row.names(data))
fd <- new('AnnotatedDataFrame', data = fData)
...
cth <- newCellTypeHierarchy()
Myc<-FetchData(pbmc2,vars = "Myc")
cth <- addCellType(cth, "Myc", classify_func =
                     function(x) { x[Myc_id,] >= mean(Myc$Myc) })
cth <- addCellType(cth, "no_Myc", classify_func =
                     function(x) { x[Myc_id,] < mean(Myc$Myc) })
