...
cth <- addCellType(cth, "Myc_Irf4_ly75", classify_func =
                     function(x) { x[Myc_id,] >= mean(Myc$Myc) & x[Irf4_id,] >= mean(Irf4$Irf4) & x[Ly75_id,] >= mean(Ly75$Ly75) })
...
candidate_clustering_genes <-
  row.names(subset(marker_diff, qval < 0.000000001))
marker_spec <-
