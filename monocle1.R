...
mtor_1<-read.table("D:/I/MAC/Memory_B cells/PENG_LEUCINE_DEPRIVATION_DN_new.txt",sep = "\t",header = TRUE)
...
mtor_2<-read.table("D:/I/MAC/Memory_B cells/PENG_RAPAMYCIN_RESPONSE_DN_new.txt",sep = "\t",header = TRUE)
...
mtor_3<-read.table("D:/I/MAC/Memory_B cells/MTORC1_hallmarks.txt",sep = "\t",header = TRUE)
...
Glutamine_1<-read.table("D:/I/MAC/Memory_B cells/GO_BP_Glutamine.txt",sep = "\t",header = TRUE)
...
Kdm6b_id <- row.names(subset(fData(monocle_cds), gene_short_name == "Kdm6b"))

cth <- newCellTypeHierarchy()
Myc<-FetchData(pbmc,vars = "Myc")
Irf4<-FetchData(pbmc,vars = "Irf4")
Ly75<-FetchData(pbmc,vars = "Ly75")
Kdm6b<-FetchData(pbmc,vars = "Kdm6b")
...
cth <- addCellType(cth, "Kdm6b", classify_func =
                     function(x) {  x[Kdm6b_id,] >= mean(Kdm6b$Kdm6b)})
...
monocle_cds <- reduceDimension(monocle_cds, max_components = 3, num_dim = 7,
                        norm_method = 'log',
...
plot_cell_clusters(monocle_cds, 2, 3, color = "CellType",
                   markers = c("Aicda", "Cd83","Cd86","Hmmr","Mki67","Cxcr4"))

Idents(pbmc1)<-monocle_cds@phenoData@data[["Cluster"]]
VlnPlot(pbmc1,features = c("Myc","Irf4","Kdm6b","Ly75"),sort = TRUE,pt.size = 0)
Myc1<-subset(pbmc1,idents="9")
pbmc2<-subset(pbmc1,idents = c("3")) #Myc cluster9, cluster3
#remove(cth,data,fd,fData,Irf4,Kdm6b,Ly75,marker_diff,marker_spec,monocle_cds,monocle_cds_1,Myc,pd,candidate_clustering_genes,expressed_genes,counts,Features,semisup_clustering_genes,Irf4_id,Kdm6b_id,Ly75_id,Myc_id,percentmt)
DotPlot(pbmc1,features=c("Myc","Irf4","Ly75","Kdm6b")) + RotatedAxis()
remove(pbmc)
Myc<-Myc1[rownames(Myc1)=="Myc",]
Myc_count<-as.data.frame(t(as.data.frame(Myc@assays$RNA@counts)))
mean(Myc_count$Myc)
median(Myc_count$Myc)
Myc2<-pbmc2[rownames(pbmc2)=="Myc",]
Myc2_count<-as.data.frame(t(as.data.frame(Myc2@assays$RNA@counts)))
mean(Myc2_count$Myc)
median(Myc2_count$Myc)
Myc_hi<-as.data.frame(Myc_count[Myc_count$Myc>=mean(Myc_count$Myc),])
Myc2_hi<-as.data.frame(Myc2_count[Myc2_count$Myc>=mean(Myc2_count$Myc),])
