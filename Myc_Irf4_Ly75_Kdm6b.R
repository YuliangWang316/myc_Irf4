...
cth <- addCellType(cth, "Myc_Irf4_ly75_kdm6b", classify_func =
                     function(x) { x[Myc_id,] >= mean(Myc$Myc) & x[Irf4_id,] >= mean(Irf4$Irf4) & x[Ly75_id,] >= mean(Ly75$Ly75) & x[Kdm6b_id,] >= mean(Kdm6b$Kdm6b) })
...
candidate_clustering_genes <-
  row.names(subset(marker_diff, qval < 0.0000001))
marker_spec <-
