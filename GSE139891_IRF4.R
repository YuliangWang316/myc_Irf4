    ...
    for (i in 1:50) {
      for (j in seq(0.1,6,by=0.1)) {
        ...
        if (levels(MYC$ident)[1] == levels(LY75$ident)[1]){
          if (levels(MYC$ident)[1] == levels(IRF4$ident)[1]){
...
remove(KDM6B,LY75,MYC,P,PRDM1,all.genes,pbmcs,h,i,idents,j,k,noise,IRF4,S)
...
pbmcn <- FindClusters(pbmcn, resolution = 0.3, verbose=F)
DotPlot(pbmcn, features = c("MYC","PRDM1","KDM6B","LY75","Mtor_1","Mtor_2","Mtor_3")) + RotatedAxis()
